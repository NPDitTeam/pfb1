...
class AccountMoveLine(models.Model):
    ...
    def write(self, vals):
        if self._context.get('skip_tax_invoice_write'):
        ...
        for line in self:
            ...
            if move.move_type in ['out_invoice', 'out_refund'] and move.state == 'draft':
                if line.tax_line_id and not line.manual_tax_invoice:
                    # ลบเก่าก่อน
                    line.tax_invoice_ids.with_context(force_remove_tax_invoice=True).unlink()

                    sign = -1 if move.move_type == 'out_refund' else 1

                    # สร้างใหม่แบบ clean พร้อม context ป้องกัน recursion
                    taxinv = TaxInvoice.with_context(skip_tax_invoice_write=True).create({
                        "move_id": move.id,
                        "move_line_id": line.id,
                        "partner_id": line.partner_id.id,
                        "tax_base_amount": sign * abs(line.tax_base_amount),
                    ...
                    line.with_context(skip_tax_invoice_write=True).write({
                        'tax_invoice_ids': [(6, 0, [taxinv.id])]
                    })
    ...
    #         move = line.move_id
    #
    #         # เงื่อนไข: เอกสารต้องอยู่ใน draft และมีภาษี
    #         if move.state == 'draft' and line.tax_line_id and not line.manual_tax_invoice:
    #
    #             # ลบของเดิมก่อน
    #             line.tax_invoice_ids.with_context(force_remove_tax_invoice=True).unlink()
    #
    #             # คำนวณ sign จากประเภทใบ
    #             tax_use = line.tax_line_id.type_tax_use
    ...
    #                 sign = -1
    #             elif move_type == 'entry' and move.reversed_entry_id:
    #                 # entry ที่เป็น reversal ต้องใช้ sign ตรงข้าม
    #                 sign = -1
    #
    #             # สร้างใหม่
    #             taxinv = TaxInvoice.with_context(skip_tax_invoice_write=True).create({
