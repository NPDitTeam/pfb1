...
class AccountCutoff(models.Model):
    ...
    is_calcurate_by_month = fields.Boolean('Calculate BY Month')
    total_month = fields.Integer('Total Month',default='1')
    is_calcurate_end_month = fields.Boolean('Calculate BY End Month')
    ...
    def _prepare_month_cutoff_line(self, aml, mapping):
        line_obj = self.env["account.cutoff.line"]
        self.ensure_one()
        # total_days = (aml.end_date - aml.start_date).days + 1

        end_date = self.end_date < aml.end_date and  self.end_date or aml.end_date
        # total_months = (end_date.year - aml.start_date.year) * 12 + (end_date.month - aml.start_date.month) + 1
        total_months = self.total_month
        # assert total_months > 0, "Should never happen. Total days should always be > 0"
        ...
        # count_month = 0
        cutoff_amount = aml.balance / total_months
    ...
    def _prepare_end_month_cutoff_line(self, aml, mapping):
        line_obj = self.env["account.cutoff.line"]
        self.ensure_one()
        # total_days = (aml.end_date - aml.start_date).days + 1
    ...
    def get_lines(self):
        ...
        for aml in amls:
            if self.is_calcurate_by_month:
                self._prepare_month_cutoff_line(aml,mapping)
            elif self.is_calcurate_end_month:
                self._prepare_end_month_cutoff_line(aml,mapping)
            else:
...
class AccountCutoffLine(models.Model):
    ...
    total_month = fields.Integer(readonly=True)
