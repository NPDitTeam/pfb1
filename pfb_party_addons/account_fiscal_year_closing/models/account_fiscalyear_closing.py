...
class AccountFiscalyearClosing(models.Model):
    ...
    def _onchange_year(self):
        if self.year > 9998 or self.year < 999:
            raise ValidationError(_("Year is not correct"))
...
class AccountFiscalyearClosingMapping(models.Model):
    ...
    def create(self, vals):
        if "dest_account_id" in vals:
            vals["dest_account_id"] = isinstance(vals["dest_account_id"], int) \
                                      and vals["dest_account_id"] \
                                      or vals["dest_account_id"][0]
