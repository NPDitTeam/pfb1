...
class InsPartnerAgeing(models.TransientModel):
    ...
    hide_partner_type = fields.Boolean(string="Hide Partner Type", compute="_compute_hide_partner_type", store=True)
    branch_id = fields.Many2one('res.branch', string="สาขา")

    @api.depends('as_on_date')
    def _compute_hide_partner_type(self):
        """ ซ่อน partner_type หาก partner_ageing_show เป็น False """
        user = self.env.user
        for record in self:
            record.hide_partner_type = not user.partner_ageing_show

    @api.onchange('as_on_date', 'branch_id')
    def onchange_as_on_date(self):
        print("as_on_date:", self.as_on_date)

        user = self.env.user
        print("user.partner_ageing_show:", user.partner_ageing_show)
        print("self.branch_id.id:", self.branch_id.id)

        allowed_partners = []

        if not user.partner_ageing_show:
            # ถ้า partner_ageing_show เป็น False ให้แสดงเฉพาะ partner_id ที่เกี่ยวข้องกับ branch_id ของ user
            allowed_partners = self.env['account.move'].search([
                ('payment_state', '=', 'not_paid'),
                '|',  # <-- OR เงื่อนไขสองอันด้านล่างนี้
                ('branch_id', '=', user.branch_id.id),
                ('partner_id.branch_ids', 'in', user.branch_id.id),
                ('move_type', '=', 'out_invoice')
            ]).mapped('partner_id').ids
            # ดึง partner_id จาก account.move
            user_branch_name = user.branch_id.name if user.branch_id else "ไม่ระบุ"
            # **แสดง Popup ถ้าไม่มี partner_ids**
            if not allowed_partners:
                return {
                    'warning': {
                        'title': "ไม่มีลูกค้าค้างชำระ",
                        'message': f"ไม่พบลูกค้าที่มีใบแจ้งหนี้ค้างชำระสำหรับสาขาที่เลือก (คุณอยู่ที่: {user_branch_name}) กรุณาตรวจสอบอีกครั้ง!"
                    }
                }
        else:
            # ถ้า partner_ageing_show เป็น True ให้แสดง partner_id ของใบแจ้งหนี้ที่ยังไม่ชำระทั้งหมด
            if self.branch_id:
                allowed_partners = self.env['account.move'].search([
                    ('payment_state', '=', 'not_paid'),
                    '|',
                    ('branch_id', '=', self.branch_id.id),
                    ('partner_id.branch_ids', 'in', self.branch_id.id),
                    ('move_type', '=', 'out_invoice')
                ]).mapped('partner_id').ids  # ดึง partner_id จาก account.move
                user_branch_name = self.branch_id.name if self.branch_id else "ไม่ระบุ"
                # **แสดง Popup ถ้าไม่มี partner_ids**
                if not allowed_partners:
                    return {
                        'warning': {
        ...
        print("Filtered partner_ids:", allowed_partners)

        # **อัปเดต partner_ids อัตโนมัติ**
        self.partner_ids = [(6, 0, allowed_partners)]



        return {'domain': {'partner_ids': [('id', 'in', allowed_partners)]}}
