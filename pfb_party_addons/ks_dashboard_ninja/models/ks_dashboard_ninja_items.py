...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_data_format = fields.Selection([
        ...
        ('exact', 'Exact Value')
    ], string='Number System', default='global', help="To Change the number format showing in chart to given option")
    ...
    filter_lost_won = fields.Selection([
        ('not', 'Not specified'),
        ('lost', 'Lost'),
        ('won', 'Won')
    ], string="Filters Lost Won", default="not", store=True)
...
# **************************************** NPD ****************************************
            if rec.filter_lost_won == 'not':
                ks_record_count = rec.ks_fetch_model_data(rec.ks_model_name, rec.ks_domain, 'search_count', rec, domain)

            if rec.filter_lost_won == 'lost':
                domain = [
                    ('probability', '=', 0),
                    ('active', '=', False),
                ...
                debt_leads_count = rec.env['crm.lead'].search_count(domain)

                ks_record_count = debt_leads_count

            if rec.filter_lost_won == 'won':
                domain = [
                    ('probability', '>=', 100),
                    ('active', '=', True),
                    ('type', '=', 'opportunity')
                ...
                ks_record_count = debt_leads_count-1
# **************************************** NPD ****************************************

        elif rec.ks_record_count_type in ['sum',
                                          'average'] and rec.ks_record_field and rec.ks_dashboard_item_type != 'ks_list_view':
            ks_records_grouped_data = rec.ks_fetch_model_data(rec.ks_model_name, rec.ks_domain, 'read_group', rec, domain)
    ...
    def ks_convert_into_proper_domain(self, ks_domain, rec, domain=[]):
        ...
        if ks_domain and "%CIDS" in ks_domain:
            ks_domain = ks_domain.replace('"%CIDS"', str(self.env.user.company_id.id))
            if self.env.user.company_ids:
                self.env.companies = self.env.user.company_ids
                if self._context.get('allowed_company_ids', False):
                    for ks_id in self.env.user.company_ids.ids:
                        self._context['allowed_company_ids'].append(ks_id)
        if ks_domain and "%COMPANYIDS" in ks_domain:
            if self.env.user.company_ids:
                self.env.companies = self.env.user.company_ids
                if self._context.get('allowed_company_ids', False):
                    for ks_id in self.env.user.company_ids.ids:
                        self._context['allowed_company_ids'].append(ks_id)
            ks_domain = ks_domain.replace('"%COMPANYIDS"', str(self.env.user.company_ids.ids))
    ...
    @api.depends('ks_domain', 'ks_dashboard_item_type','ks_pagination_limit', 'ks_model_id', 'ks_sort_by_field', 'ks_sort_by_order',
    ...
    def _ksGetRecordCount_2(self, domain=[]):
        ...
        if rec.ks_record_count_type_2 == 'count':
            ks_record_count = rec.ks_fetch_model_data_2(rec.ks_model_name_2, rec.ks_domain_2, 'search_count', rec, domain)
    ...
    def ks_convert_into_proper_domain_2(self, ks_domain_2, rec, domain=[]):
        ...
        if ks_domain_2 and "%CIDS" in ks_domain_2:
            ks_domain_2 = ks_domain_2.replace("%CIDS", str(self.env.user.company_id.id))
            if self.env.user.company_ids:
                self.env.companies = self.env.user.company_ids
                if self._context.get('allowed_company_ids', False):
                    for ks_id in self.env.user.company_ids.ids:
                        self._context['allowed_company_ids'].append(ks_id)
        if ks_domain_2 and "%COMPANYIDS" in ks_domain_2:
            if self.env.user.company_ids:
                self.env.companies = self.env.user.company_ids
                if self._context.get('allowed_company_ids', False):
                    for ks_id in self.env.user.company_ids.ids:
                        self._context['allowed_company_ids'].append(ks_id)
            ks_domain_2 = ks_domain_2.replace('"%COMPANYIDS"', str(self.env.user.company_ids.ids))
