...
class PurchaseOrder(models.Model):
    ...
    def write(self, data):
        ...
        for rec in self:
            # ตรวจสอบ งวดเงิน งวดงาน
            if rec.invoice_plan_ids:
                percent = 0
                percent = sum(line.percent for line in rec.invoice_plan_ids)
                Decimal = self.env['decimal.precision']
                prec = Decimal.precision_get('Product Unit of Measure')
                percent = float_round(percent, prec)
                if percent != 100:
    ...
    def _invoice_plan_count(self):
        ...
        for line_order in self.order_line:
            if line_order.is_deposit:
                count_deposit += 1
        if self.invoice_count:
            count_invoice = self.invoice_count - count_deposit
        if len(self.invoice_plan_ids) == count_invoice:
            self.hide_button = True
            # service
            for line_order in self.order_line:
                if line_order.product_id.purchase_method == 'receive' and line_order.product_id.type == 'service':
                    if line_order.qty_invoiced != 0:
                        line_order.qty_to_invoice = 0
                        line_order.qty_invoiced = 1
        else:
            ...
            for line_order in self.order_line:
                if line_order.product_id.purchase_method == 'receive' and line_order.product_id.type == 'service':
                    if line_order.qty_invoiced != 0:
                        line_order.qty_to_invoice = 1
                        line_order.qty_invoiced = 0
    ...
    def _compute_ip_total(self):
        for rec in self:
            ...
            for line in rec.invoice_plan_ids:
                ...
                print(line.amount)
                print('------')
            rec.ip_total_amount = sum(line.amount for line in rec.invoice_plan_ids)
...
class PurchaseInvoicePlan(models.Model):
        ...
        for rec in self:
            ...
            for line in rec.purchase_id.order_line:
                if line.product_id and line.price_subtotal != 0:
                    service = line.product_id.purchase_method
            ...
            #         print('if40')
            #         if rec.amount == 0:
            #             rec.amount = rec.purchase_id.amount_total - prev_amount
            #             rec.percent = 100 - prev_percent
            #         print(rec.purchase_id.amount_total - prev_amount)
            #     else:
            #         print('if401')
            ...
            if service == 'receive':
                print('if41')
                rec.amount = round(round(rec.percent * rec.purchase_id.amount_total, 2) / 100, 2)
                # rec.percent = 100 - prev_percent
            else:
                print('if42')
    ...
    def _update_new_quantity(self, line, percent):
        ...
        # แก้ราคาในกรณีให้รองรับ งวดงานของ service
        if line.product_id.purchase_method == 'receive' and line.product_id.type == 'service':
            installments = self.purchase_id.invoice_plan_ids.filtered(
                lambda l: l.to_invoice and not l.invoiced
            )
            if line.tax_ids and not line.tax_ids.price_include:
                print('7/107')
                percent = 7 / 107
                price = installments.amount - (installments.amount * percent)
                line.write({"price_unit": price})
            else:
                print('100%')
                price = installments.amount
                line.write({"price_unit": price})
        if line.purchase_line_id.is_deposit:
            line.write({"quantity": 1})

        if not line.purchase_line_id.is_deposit and line.purchase_line_id.price_subtotal == 0:
            line.write({"price_unit": (line.purchase_line_id.price_unit * (percent / 100)) * -1})
            line.write({"quantity": 1})

    @api.model
    def _get_plan_qty(self, order_line, percent):
        if order_line.product_id.purchase_method == 'receive' and order_line.product_id.type == 'service':
            plan_qty = order_line.product_qty
        else:
