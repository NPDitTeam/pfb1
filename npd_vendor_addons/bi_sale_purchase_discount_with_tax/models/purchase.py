...
class purchase_order(models.Model):
    ...
    @api.depends('order_line','order_line.price_total','order_line.price_subtotal',\
        ...
        'order_line.discount_method','order_line.price_tax')
                            ...
                            if line.price_unit and line.product_qty:
                                if line.discount_amount:
                                    ...
                                    if line.taxes_id:
                                        ...
                                        taxes_res = line.taxes_id.compute_all(price_unit_discount,cur_obj, line.product_qty, line.product_id)
                                ...
                                for taxes_id in line.taxes_id:
                                    if taxes_id.price_include:
                                        ...
                                        if line.discount_amount > 0 :
                                            line.discount_amt = (line.price_unit * line.product_qty) * (100/(100+taxes_amount_include)) * line.discount_amount/100                                            
                                    else:
                                        #not calculate WTH -3 % on PO
                                        if taxes_id.amount > 0:
                                            taxes_amount_execlude = taxes_id.amount
                            ...
                            if line.taxes_id:
                                for taxes_id in line.taxes_id:
                                    if taxes_id.price_include:
                                        ...
                                        if taxes_id.amount > 0:
                                            taxes_amount_execlude = taxes_id.amount
                            taxes_res = line.taxes_id.compute_all(line.price_unit,cur_obj, line.product_qty, line.product_id)
                            stwd = 0.0
                            stwd = (line.price_unit * line.product_qty) * (100/(100+taxes_amount_include))
                            line.price_subtotal_without_discount =  stwd
                            line.price_tax = sum(t.get('amount', 0.00000) for t in taxes_res.get('taxes', []))
    ...
    def action_create_invoice(self):
        ...
        return super().action_view_invoice(invoices=moves)             
...
class purchase_order_line(models.Model):
    ...
    def whenchangeproduct(self):
        for each in self:
            if each.order_id:
                if each.order_id.partner_id:
                    each.discount_amount = each.order_id.partner_id.default_supplierinfo_discount
                    if each.order_id.partner_id.default_supplierinfo_discount > 0:
                       each.discount_method = 'per'
    ...
    @api.depends('product_qty', 'price_unit', 'taxes_id','discount_method','discount_amount','discount_type','price_subtotal_without_discount')
    ...
    def _prepare_account_move_line(self):
        ...
        if line.discount_method == 'fix':
            ...
            if (line.product_qty * line.price_unit) > 0:
                discount_percent = line.discount_amount/((line.product_qty * line.price_unit) *100)
        #res.update({'discount_method':self.discount_method,'discount_amount':self.discount_amount,'quantity':self.product_qty,'discount':discount_percent})
        res.update({'discount_method':self.discount_method,'discount_amount':self.discount_amount,'discount':discount_percent})
        return res
    ...
    #--------accounting_standard_odoo14/issues/328 
    @api.onchange("product_qty", 'discount',"price_unit", 'discount_amount','discount_method','price_tax','taxes_id')
    def _onchange_purchase_order_line(self):
        #for order in self.mapped('order_id'):
            #for line in order.order_line:
            for line in self:
                #line.price_subtotal = (line.product_qty*line.price_unit)- line.discount_amount - ((line.product_qty*line.price_unit)*line.discount/100)
                if line.discount_method == 'fix':
                        cur_obj = self.env['res.currency']
                        taxes_amount_include = 0.0
                        taxes_amount_execlude = 0.0
                        if line.discount_amount:
                            if line.price_unit and line.product_qty:
                                if line.taxes_id:
                                    ...
                                    #discount_percent = line.discount_amount/((line.product_qty * line.price_unit) *100 line.discount_amount)
                                    #discount_percent = line.discount_amount/((line.product_qty * line.price_unit) *100)
                                    stwd = 0.0
                                    stwd = (line.price_unit * line.product_qty) * (100/(100+taxes_amount_include))
                                    discount_percent = (line.discount_amount/(line.product_qty * stwd))*100
