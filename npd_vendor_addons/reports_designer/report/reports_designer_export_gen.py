...
class ReportsDesignerExportGen(models.TransientModel):
    _name = 'reports_designer_export_gen'
    _description = " "
    def create_conf(self, datas=None):
        ...
        for active_id in active_ids_all:
            ...
            ReportsDesignerFields_Array = (
                                        'sequence',
                                        'cell',
                                        'reports_designer_section_id',
                                        'group_by',
                                        'aggregate',
                                        'aggregate_id',
                                        'having_operator',
            ...
            ReportsDesignerTemlateFields_Array = (
                                        'name',
                                        'type',
                                        'datas',
                                        'res_id',
            ...
            node_record_ReportsDesigner = etree.Element('record')
            node_data.append(node_record_ReportsDesigner)
            node_record_ReportsDesigner.attrib['id'] = ''.join([report_obj._name.replace('.', '_'),'_', str(randint(100000000, 999999999))])
            node_record_ReportsDesigner.attrib['model'] = report_obj._name
            for f in ReportsDesigner_Array:
                ...
                if f == 'root_model_id':
                    ...
                    node_record_ReportsDesigner.append(node_field)
                    node_field.attrib['name'] = f
                    node_field.attrib['model'] = "reports.designer"
            ...
            for reports_designer_param_obj in report_obj.reports_designer_param_ids:
                node_record_param = etree.Element('record')
                node_data.append(node_record_param)
                node_record_param.attrib['id'] = ''.join([reports_designer_param_obj._name.replace('.', '_'),'_', str(randint(100000000, 999999999))])
                node_record_param.attrib['model'] = reports_designer_param_obj._name
                list_external_id_params += ((reports_designer_param_obj.code, node_record_param.attrib['id'],),) 
                for f in ReportsDesignerParam_Array:
                    field = reports_designer_param_obj.fields_get(f)[f]    
                    if field['type'] == 'many2one':
                        field_relation = field['relation']
                        external_id = self.env['ir.model.data'].search([('res_id', '=', reports_designer_param_obj[f].id),('model', '=', field_relation)], limit=1).name
                        module_name = self.env['ir.model.data'].search([('res_id', '=', reports_designer_param_obj[f].id),('model', '=', field_relation)], limit=1).module
                        module_dependencies.append(module_name)
                        if f == 'reports_designer_id':
                            params_str = node_record_ReportsDesigner.attrib['id']
                        else:
                            if external_id:
                                params_str = ''.join([module_name,'.', external_id,])
                            else:
                        ...
                        node_record_param.append(node_field)
                        node_field.attrib['name'] = f
                        if reports_designer_param_obj[f] != False:
                            node_field.text = str(reports_designer_param_obj[f])
                        else:
                            node_field.attrib['eval'] = str(reports_designer_param_obj[f])                
            for section_obj in report_obj.reports_designer_section_ids:
                self.get_report_section(node_record_ReportsDesigner.attrib['id'], node_data, section_obj, ReportsDesignerSection_Array, ReportsDesignerFields_Array, None, module_dependencies, list_external_id_params)
            if len(list_external_id_params):
                node_record_ReportsDesigner_update_1 = etree.Element('record')
                node_data.append(node_record_ReportsDesigner_update_1)
                node_record_ReportsDesigner_update_1.attrib['id'] = node_record_ReportsDesigner.attrib['id']
                node_record_ReportsDesigner_update_1.attrib['model'] = report_obj._name
                node_field = etree.Element('field')
                node_record_ReportsDesigner_update_1.append(node_field)
                node_field.attrib['name'] = "reports_designer_param_ids"
                eval_str = ''
            ...
            module_dependencies_txt = 'Module Dependencies: ' +  str([i for i in list(set(module_dependencies)) if i != False ])
            ...
            if datas['multiple_export']:
                ...
                file_obj = {
                    ...
                    'url': '/reports_designer_export?id=%s' % (new_attach.id,),
                    'target': 'self',
                }
                return file_obj
        ...
        zip_file.close()        
        out_file_name_zip = 'reports_designer_configurations.zip'
        ...
        return file_obj
    def get_report_section(self, reports_designer_ref, node_data, section_obj, ReportsDesignerSection_FieldsArray, ReportsDesignerFields_FieldsArray, parent_ref=None, module_dependencies=[], list_external_id_params=()):
        ...
        for f in ReportsDesignerSection_FieldsArray:
            ...
            if f == 'root_model_id':
                ...
                node_field.attrib['name'] = f
                node_field.attrib['model'] = "reports.designer.section"
                node_field.attrib['eval'] = eval_str
                field_relation = field['relation']
            ...
            if field['type'] == 'many2one':
                ...
                module_dependencies.append(module_name)
                if f == 'reports_designer_id':
                    if len(section_obj.reports_designer_id):
                        params_str = reports_designer_ref
                    else:
                        continue
                elif f == 'parent_id': 
                    if len(section_obj.parent_id):
                        ...
                        node_field.attrib['eval'] = '[]'
                        continue
                elif f == 'root_model_related_field_id': 
                        if len(section_obj.parent_id):
                            if not external_id:
                                node_field = etree.Element('field')
                                node_record_section.append(node_field)
                                node_field.attrib['name'] = f
                                node_field.attrib['search'] = ''.join(["[('model', '=', '", section_obj.root_model_related_field_id.model, "'), ('name', '=', '", section_obj.root_model_related_field_id.name,"')]"])
                                continue
                            else:                            
                                params_str = ''.join([module_name,'.', external_id,])
                        else:
        ...
        for excel_field_obj in section_obj.reports_designer_fields_ids:
            ...
            for f in ReportsDesignerFields_FieldsArray:
                ...
                if field['type'] == 'many2one':
                    ...
                    module_dependencies.append(module_name)
                    if f == 'reports_designer_section_id':
        ...
        for child_section_obj in section_obj.children_ids:
            self.get_report_section(reports_designer_ref, node_data, child_section_obj, ReportsDesignerSection_FieldsArray, ReportsDesignerFields_FieldsArray, node_record_section.attrib['id'], module_dependencies, list_external_id_params)
