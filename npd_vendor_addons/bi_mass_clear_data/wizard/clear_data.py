    ...
    def clear_data(self):
        for record in self:
            ...
            if record.sale_and_transfer:
                ...
                if sale_model:
                    ...
                    for sale in sale_obj:
                        sale.action_cancel()
                        if sale.tasks_ids:
                            for task in sale.tasks_ids:
                                task.sale_line_id.unlink()
                    ...
                    for picking in picking_obj:
                        ...
                        picking.with_context(prefetch_fields=False).mapped('move_lines').unlink()  # Checks if moves are not done
                    picking_obj.unlink()
            ...
            if record.journal_entry:
                withholding_obj = self.env['withholding.tax.cert'].sudo().search([])
                withholding_obj.unlink()
                # asset_obj = self.env['account.asset'].sudo().search([])
                ...
                if move_model:
                    ...
                    journal_entry_obj.with_context(force_delete=True, unlink_from_asset=True).unlink()

            if record.invoice_payment_journal:
                asset_repair = self.env['asset.repair.order'].sudo().search([])
                for repair in asset_repair:
                    repair.unlink()
                asset_obj = self.env['account.asset'].sudo().search([])
                for asset in asset_obj:
                    if asset.state == 'open':
                        for line in asset.depreciation_line_ids:
                            if line.move_check:
                                print(">>>>>>>>",line.move_id)
                                line.move_id.with_context(force_delete=True, unlink_from_asset=True).unlink()
            ...
            if record.all_accounting_data:
                ...
                if move_model:
                    ...
                    payment_acquirer_obj = self.env['payment.acquirer'].sudo().search([])
                    ...
                    account_tax_report_obj = self.env['account.tax.report'].sudo().search([])
                    account_tax_report_obj.unlink()

                    account_bank_statement_obj = self.env['account.bank.statement'].sudo().search([])
                    for bank_statement in account_bank_statement_obj:
                        if bank_statement.state != 'open':
                            bank_statement.button_reopen()
