...
class InsAdvanceAgeing(models.TransientModel):
    ...
    def write(self, vals):
        ...
        ret = super(InsAdvanceAgeing, self).write(vals)
        return ret
    ...
    def get_filters(self, default_filters={}):
        ...
        filter_dict = {
            ...
            'employees_list': [(p.id, p.name) for p in employees],
            'department_list': [(p.id, p.name) for p in department_list],
    ...
    def process_filters(self):
        ...
        filters['employees_list'] = data.get('employees_list')
    ...
    # def process_detailed_data(self, offset=0, employee=0, fetch_range=FETCH_RANGE):
        '''
        ...
        if partner:
            sql = """
                ...
                INNER JOIN
                    account_advance AS ad ON ad.move_id = m.id 
                LEFT JOIN
                    hr_employee AS emp ON emp.id = ad.employee_id
                LEFT JOIN
                ...
                WHERE
                    l.balance <> 0
                    AND ad.state_remain = 'Wait Clear'
                    AND ad.due_date_clear	<= '%s'
                    AND emp.id = '%s'
                """ % (as_on_date, partner)
            self.env.cr.execute(sql)
            count = self.env.cr.fetchone()[0]

            SELECT = """SELECT ad.name AS move_name,
                                m.id AS move_id,
                                l.date AS date,
                                l.date_maturity AS date_maturity, 
                                ad.due_date_clear  AS due_date,
            ...
            sql = """
                    ...
                    WHERE
                        ...
                        AND emp.id = '%s'
                    GROUP BY
                        l.move_id, l.date, l.date_maturity,ad.due_date_clear, m.id, ad.name, j.name, a.name, cc.id
                    OFFSET %s ROWS
                    FETCH FIRST %s ROWS ONLY
                """%(as_on_date, partner, offset, fetch_range)
    ...
    def process_data(self):
        ...
        if self.department_id:
            employee_ids = self.env['hr.department'].search(('id','=',self.department_id.id)).member_ids
            if employee_ids:
               employee_ids = employee_ids
        else:
            employee_ids = self.employee_ids or self.env['hr.employee'].search(domain)

        as_on_date = self.as_on_date
        company_currency_id = self.env.company.currency_id.id
        employee_dict = {}
        for employee in employee_ids:
            employee_dict.update({employee.id:{}})

        employee_dict.update({'Total': {}})
        for period in period_dict:
            employee_dict['Total'].update({period_dict[period]['name']: 0.0})
        employee_dict['Total'].update({'total': 0.0, 'partner_name': 'ZZZZZZZZZ'})
        employee_dict['Total'].update({'company_currency_id': company_currency_id})

        for employee in employee_ids:
            employee_dict[employee.id].update({'partner_name':employee.name})
            total_balance = 0.0
            ...
            if count:
                for period in period_dict:
                    ...
                    # where = " AND l.date <= '%s' AND l.partner_id = %s AND COALESCE(l.date_maturity,l.date) "%(as_on_date, employee.id)
                    where = " AND l.date <= '%s' AND emp.id = '%s'  AND COALESCE(l.date_maturity,l.date) "%(as_on_date,employee.id)
                    if period_dict[period].get('start') and period_dict[period].get('stop'):
                    ...
                    amount = 0.0
                    GROUPBY = 'GROUP BY l.move_id'
                    self.env.cr.execute(sql + where + GROUPBY)
                    ...
                    employee_dict[employee.id].update({period_dict[period]['name']: 0})
                    employee_dict['Total'][period_dict[period]['name']] += 0

                employee_dict[employee.id].update({'count': count})
                employee_dict[employee.id].update({'pages': self.get_page_list(count)})
                employee_dict[employee.id].update({'single_page': True if count <= FETCH_RANGE else False})
                employee_dict[employee.id].update({'total': total_balance})
                employee_dict['Total']['total'] += total_balance
                employee_dict[employee.id].update({'company_currency_id': company_currency_id})
                employee_dict['Total'].update({'company_currency_id': company_currency_id})
            else:
                employee_dict.pop(employee.id, None)
        return period_dict, employee_dict
    ...
    def get_xlsx_report(self, data, response):
        ...
        # Get record and data
        record = self.env['ins.advance.ageing'].browse(data.get('id', [])) or False
