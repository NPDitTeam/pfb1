    ...
    def _compute_wa_accepted(self):
        for order in self:
            ...
            if order.invoice_plan_ids:
                check = False
                wa = order.wa_count
                ip = len(order.invoice_plan_ids)
                for po_line in order.order_line:
                    if po_line.product_id.purchase_method == 'receive' and po_line.product_id.type == 'service' and wa != ip:
                        order.wa_accepted = check
                    else:
                        if wa != ip:
                            order.wa_accepted = check
                        else:
                            order.wa_accepted = False
...
class PurchaseOrderLine(models.Model):
    ...
    def _get_product_qty(self):
        print('_get_product_qty')
        print(self.product_qty - sum(
            wa_line.product_qty
    ...
    def _prepare_account_move_line(self, move=False):
        ...
        if self.product_id.purchase_method == 'receive' and self.product_id.type == 'service':
            installments = self.order_id.invoice_plan_ids.filtered(
                lambda l: l.to_invoice and not l.invoiced
            )
            if self.taxes_id and not self.taxes_id.price_include:
                percent = 7 / 107
        ...
        if move and move.wa_id:
            ...
            print('wa_line_product_qty')
            print(wa_line.product_qty)
