    ...
    def _compute_depreciation(self):
        for asset in self:
            lines = asset.depreciation_line_ids.filtered(
                ...
                and (l.init_entry or l.move_check or l.is_not_create_move)
            )
        ...
        for entry in table[table_i_start:]:
            for line in entry["lines"][line_i_start:]:
                ...
                if amount:
                    ...
                    if self.date_purchase and self.date_purchase >= line["date"]:
                        vals.update({"is_not_create_move": "True"})
