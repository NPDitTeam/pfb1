...
class NonMovingProductWizard(models.Model):
    ...
    def get_lines(self, warehouse):
        ...
        for i in self.warehouse:
            ...
            all_product_id = self.env['product.product'].search([('location_id','=',i.lot_stock_id.id)])
            all_list = []
            ...
            for pro in  final_list_pro:
                ...
                self._cr.execute("""select max(id) from sale_order_line where product_id ='%s' and company_id = '%s';""",(pro,i.company_id.id))
            ...
            for line in sale_order_line_list:
                    ...
                    stock = stock_data.search([('product_id','=',sale_order_line_obj.product_id.id),('location_id','=',i.lot_stock_id.id)])
                    for record in stock:
                        if record.quantity:
    ...
    def print_exl_report(self):
        ...
        export_id = self.env['non.moving.report.excel'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
        res = {
                        'view_mode': 'form',
                        'res_id': export_id.id,
