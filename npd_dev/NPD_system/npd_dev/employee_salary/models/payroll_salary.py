...
class PayrollSalary(models.Model):
    ...
    def _company_info_by_key(self, company):
        ...
        addr_ladlumkaew = ('ที่อยู่ 47/4 หมู่ที่ 2 ตำบลลาดหลุมแก้ว '
                           'อำเภอลาดหลุมแก้ว จ.ปทุมธานี 12140')
        addr_samphran = ('ที่อยู่ 36/10 หมู่ที่ 2 ตำบลบางเตย '
                         'อำเภอสามพราน จ.นครปฐม 73210')
        mapping = {
            'นภดลเอสกรุ๊ปจำกัด': {
    ...
    def _prepare_ot_lines(self):
        ...
        # ✅ วันหยุดนักขัตฤกษ์ของทุกปีที่รอบคาบเกี่ยว (รอบ ม.ค. คาบ ธ.ค. ปีก่อน)
        holidays = self._official_holidays_for_cycle(tag='OT')
    ...
    def _official_holidays_for_cycle(self, tag='PAYROLL'):
        """วันหยุดนักขัตฤกษ์ของ "ทุกปีที่รอบคาบเกี่ยว"

        payroll.holiday เก็บแยกเป็นเทมเพลตรายปี แต่รอบเดือนคาบ 2 เดือน
        และรอบเดือนมกราคมคาบ 2 ปี (25/12 ปีก่อน ถึง 24/01 ปีนี้)
        ถ้าดึงแค่ปีเดียวตามเดิม วันหยุดฝั่งเดือน ธ.ค. จะหายไป
        เช่น 31 ธ.ค. "วันสิ้นปี" กลายเป็นวันทำงาน แล้วหักขาดงานผิดยกบริษัท

        (ส่งวันหยุดเกินช่วงไปได้ ฝั่ง PHP เทียบเฉพาะวันที่อยู่ในรอบอยู่แล้ว)
        """
        self.ensure_one()
        try:
        ...
        years = {year_int}
        if month_int == 1:
            years.add(year_int - 1)

        templates = self.env['payroll.holiday'].sudo().search(
            [('year', 'in', list(years))])
        missing = years - set(templates.mapped('year'))
        if missing:
            _logger.warning(
                "[%s] ยังไม่มีตารางวันหยุดของปี %s "
                "- วันหยุดของปีนั้นจะถูกคิดเป็นวันทำงาน", tag, sorted(missing))
        holidays = sorted(line.date.strftime('%Y-%m-%d')
                          for tmpl in templates for line in tmpl.line_ids)
        _logger.info("[%s] Holiday years=%s templates=%d holidays_count=%d",
                     tag, sorted(years), len(templates), len(holidays))
        return holidays
    ...
    def _prepare_lateness_data(self):
        ...
        # วันหยุดนักขัตฤกษ์ของทุกปีที่รอบคาบเกี่ยว (รอบ ม.ค. คาบ ธ.ค. ปีก่อน)
        official_holidays = self._official_holidays_for_cycle(tag='LATENESS')
