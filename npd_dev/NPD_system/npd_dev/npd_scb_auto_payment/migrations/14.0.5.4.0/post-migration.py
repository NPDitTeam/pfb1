# -*- coding: utf-8 -*-
u"""ลบชื่อผู้โอนที่ระบบจำผิดไว้ แล้วให้ตรวจใบที่ได้รับผลกระทบใหม่

ระบบจำชื่อผู้โอนอัตโนมัติทุกครั้งที่จับคู่สำเร็จ เพื่อให้ครั้งหน้าเทียบได้ทันที
แต่เดิมจำจากรายการที่เข้าบัญชี "บริษัทอื่นในเครือ" ด้วย พอจับคู่ผิดหนึ่งครั้ง
ชื่อผิดจะถูกจำไว้ถาวรแล้วย้อนกลับมาทำให้จับคู่ผิดซ้ำเรื่อย ๆ

เจอจริงบน NPD_Intertrading_New: ลูกค้า "นาย มิตร ศิริแก้ว" ถูกจำชื่อ
"บจก. มาดี อิเล็คทร" ไว้ (มาจากรายการในบัญชีของนภดล เอส กรุ๊ป) ทำให้
CUST.IN-260806-0024 จับคู่กับรายการผิดบัญชีทุกครั้งที่ตรวจ

ลบเฉพาะชื่อที่ระบบจำเอง (origin='auto') ที่มาจากรายการนอกบัญชีบริษัทเรา
ส่วนชื่อที่คนเพิ่มเองไว้ (origin='manual') ไม่แตะต้อง
"""
import logging

from odoo import api, SUPERUSER_ID

_logger = logging.getLogger(__name__)


def migrate(cr, version):
    env = api.Environment(cr, SUPERUSER_ID, {})
    Alias = env['npd.scb.counterparty.alias'].with_context(active_test=False)
    Statement = env['npd.scb.bank.statement']
    Payment = env['account.payment']

    aliases = Alias.search([('origin', '=', 'auto')])
    if not aliases:
        return

    # ชื่อบริษัท/เลขบัญชีของเรา — ใช้ชุดเดียวกับตอนตรวจสอบการโอน
    sample = Payment.search([], limit=1)
    if not sample:
        return
    names = sample._scb_own_account_names()
    numbers = sample._scb_own_account_numbers()

    own_keys = {Statement._normalize_name(n) for n in names if n}
    own_keys.discard('')
    bad = Alias.browse()
    for alias in aliases:
        # ชื่อที่จำไว้ต้องมาจากรายการในบัญชีของบริษัทเรา
        rows = Statement.search([('counterparty', '=', alias.name)], limit=5)
        if rows and not any(r.belongs_to_company(names, numbers) for r in rows):
            bad |= alias

    if not bad:
        _logger.info(u"SCB: ไม่พบชื่อผู้โอนที่จำผิดไว้")
        return

    # กันพลาด: ถ้าจะลบเกินครึ่ง แปลว่าตัวเทียบ "บัญชีของบริษัทเรา" น่าจะเพี้ยน
    # (เช่นชื่อบริษัทในระบบเขียนต่างจากในชีตมาก) ไม่ใช่ชื่อที่จำไว้ผิดจริง
    if len(bad) * 2 > len(aliases):
        _logger.warning(
            u"SCB: จะลบชื่อที่จำไว้ %s จาก %s รายการ ซึ่งมากผิดปกติ — "
            u"ข้ามการล้างไว้ก่อน กรุณาตรวจว่าชื่อบริษัท (%s) ตรงกับชื่อเจ้าของ "
            u"บัญชีในชีตหรือไม่", len(bad), len(aliases), u' / '.join(names))
        return

    partners = bad.mapped('partner_id')
    _logger.info(u"SCB: ลบชื่อผู้โอนที่จำผิดไว้ %s รายการ (ลูกค้า %s ราย): %s",
                 len(bad), len(partners),
                 u', '.join(bad.mapped('name'))[:500])
    bad.unlink()

    # ใบของลูกค้าเหล่านี้ที่เคยตรวจไปแล้ว ต้องตรวจใหม่ด้วยชื่อที่สะอาดแล้ว
    cr.execute("""
        UPDATE account_payment
           SET scb_verify_state = 'to_check', scb_verify_attempts = 0,
               scb_verify_reason = NULL, scb_verify_summary = NULL,
               scb_statement_id = NULL
         WHERE partner_id = ANY(%s)
           AND scb_verify_state IN ('success', 'other_company', 'failed')
    """, [partners.ids])
    _logger.info(u"SCB: ตั้งใบรับชำระให้ตรวจใหม่ %s ใบ", cr.rowcount)

    cr.execute("""
        UPDATE npd_scb_payment_slip s
           SET state = 'to_check', statement_id = NULL, reason = NULL
          FROM account_payment p
         WHERE p.id = s.payment_id AND p.partner_id = ANY(%s)
           AND s.state IN ('matched', 'not_found')
    """, [partners.ids])
