...
class ImportProjectCustomWizard(models.TransientModel):
    _name = "import.project.custom.cl.wizard"
    _description = "Import Project Order Wizard"
    ...
    def import_project_apply(self):
        project_cl_obj = self.env['project.custom.checklist']
        if self:
            for rec in self:
                # For CSV
                if rec.import_type == 'csv':
                    ...
                    try:
                        ...
                        skip_header = True
                        created_project_cl = False
                        created_project_cl_list = []
                        for row in myreader:
                            try:
                                if skip_header:
                                    skip_header = False
                                ...
                                if row[0] not in (None, "") and row[1] not in (None, ""):
                                    ...
                                    if row[1] != '':
                                        ...
                                        created_project_cl = project_cl_obj.create(
                                            vals)
                                        created_project_cl_list.append(
                                            created_project_cl.id)
                    ...
                    if counter > 1:
                        completed_records = len(created_project_cl_list)
