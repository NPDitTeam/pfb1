...
class ShIrAttachment(models.Model):
    ...
    def notify_task_document_expiry(self):
        ...
        notify_create_user_template = self.env.ref(
            'sh_all_in_one_pms.sh_task_document_expiry_notify_email_to_user')
        ...
        if template and company_object and company_object.sh_task_expiry_notification:
            ...
            if document_obj:
                for record in document_obj:
                    ...
                    if record.expiry_date and record.sh_is_notify:
                        if record.company_id.task_attachment_email or record.partner:
                            # On Expiry Date
                            if company_object.sh_task_on_date_notify:
                                ...
                                if datetime.strptime(str(record.expiry_date), DEFAULT_SERVER_DATE_FORMAT).date() == datetime.now().date():
                                    ...
                                    if notify_create_user_template and record.company_id.task_attachment_email:
