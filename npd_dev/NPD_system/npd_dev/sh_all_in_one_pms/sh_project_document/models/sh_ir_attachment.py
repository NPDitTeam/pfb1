...
class ShIrAttachment(models.Model):
    ...
    def notify_project_document_expiry(self):
        ...
        notify_create_user_template = self.env.ref(
            'sh_all_in_one_pms.sh_project_document_expiry_notify_email_to_user')
            ...
            if document_obj:
                for record in document_obj:
                    ...
                    if record.expiry_date and record.sh_is_notify:
                        if record.company_id.attachment_email or record.partner:
                            # On Expiry Date
                            if company_object.sh_on_date_notify:
                                if datetime.strptime(str(record.expiry_date), DEFAULT_SERVER_DATE_FORMAT).date() == datetime.now().date():
                                    ...
                                    if template and company_object.sh_is_notify_customer and record.partner:
                                        template.send_mail(
