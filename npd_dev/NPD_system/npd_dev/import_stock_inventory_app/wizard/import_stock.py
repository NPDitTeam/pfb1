...
class StockImport(models.TransientModel):
	...
	def import_file_button(self):
		...
		if self.inventory_option == 'add' :
			stock_inventory = inventory_obj.create({'name':self.name,'location_ids':[self.location_id.id],'accounting_date' : self.account_date, 'start_empty': True, 'prefill_counted_quantity': 'zero'})
			...
			if stock_inventory.state == 'draft' :
				stock_inventory.action_start()
			if stock_inventory.state == 'confirm' :
				...
				if no <= 0:
					...
					if data :
						...
						if self.import_prod_option == 'name' :
							...
							if product_list and product_rec and stock_inventory.state in ['draft','confirm']:
						...
						if lot_num_rec : 
							if stock_inventory.state in ['draft','confirm']:
				...
				if no!= 0:
						...
						if values :
							...
							if self.import_prod_option == 'name' :
								...
								if product_rec:
									product_list.append(product_rec.id)
								if product_list and stock_inventory.state in ['draft','confirm'] and product_rec:
									stock_inventory.write({'product_ids':[(6,0,product_list)]})
							...
							if self.import_prod_option == 'barcode':
								...
								if product_rec:
									product_list.append(product_rec.id)
								if product_rec and product_list and stock_inventory.state in ['draft','confirm']:
									stock_inventory.write({'product_ids':[(6,0,product_list)]})
