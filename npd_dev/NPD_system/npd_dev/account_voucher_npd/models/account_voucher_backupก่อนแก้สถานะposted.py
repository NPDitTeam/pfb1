    ...
    def _onchange_rental_return_select_id(self):
        ...
        for record in self:
            ...
            if deposit_count == 0:
                ...
                elif getattr(related_picking.sale_id.pricelist_id, 'name', '') == 'เรทเดือน':
                    dd = (1 if (fx == sx) else (fx - sx).days)
            ...
            # ) or 0.0)
            # สำหรับ total_l: กรอง reason_code_id.name == 'สินค้าหาย'
            debit_notes_l = related_picking.sale_id.invoice_ids.mapped('debit_note_ids').filtered(
                lambda dn: dn.reason_code_id and dn.reason_code_id.name == 'สินค้าหาย'
            )
            total_l = _to_float(sum(
                debit_notes_l.mapped('amount_total')
            ) or 0.0)

            # สำหรับ total_d: กรอง reason_code_id.name == 'สินค้าชำรุด'
            debit_notes_d = related_picking.sale_id.invoice_ids.mapped('debit_note_ids').filtered(
                lambda dn: dn.reason_code_id and dn.reason_code_id.name == 'สินค้าชำรุด'
            )
            total_d = _to_float(sum(
