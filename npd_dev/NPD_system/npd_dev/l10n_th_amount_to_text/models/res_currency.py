...
class Currency(models.Model):
    ...
    def _convert_currency_name_hook(self, label):
        """Hooks to add currency translate.
        Currently, there are 3 currencies: USD, EUR, and THB"""
        return CURRENCY_NAME.get(label, label)
    ...
    def amount_to_text(self, amount):
        ...
        if self.name == "THB":
            try:
                integer_text = _num2words(integer_value, lang=lang.iso_code)
                fractional_text = _num2words(fractional_value, lang=lang.iso_code) if fractional_value else ''
                return tools.ustr(f"{integer_text} บาท" + (f" {fractional_text} สตางค์" if fractional_text else ""))
            except NotImplementedError:
                integer_text = _num2words(integer_value, lang="en")
                fractional_text = _num2words(fractional_value, lang="en") if fractional_value else ''
                return tools.ustr(f"{integer_text} Baht" + (f" {fractional_text} Satang" if fractional_text else ""))
        ...
        if not self.is_zero(amount - integer_value):
            currency_subunit_label = self._convert_currency_name_hook(self.currency_subunit_label)
            amount_words += tools.ustr(" {amt_value} {amt_word}").format(
                amt_value=_num2words(fractional_value, lang=lang.iso_code),
                amt_word=currency_subunit_label,
            )
