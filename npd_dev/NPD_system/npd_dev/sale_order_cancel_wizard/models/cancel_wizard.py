...
class SaleOrder(models.Model):
    ...
    freelance_salesperson_id = fields.Many2one(
        'res.partner', string='Freelance Salesperson', copy=False,
    )

    # สิทธิ์แสดงปุ่มยกเลิกรายการเช่า (cancel_rent_same_day)
    show_cancel_wizard_button = fields.Boolean(
        compute='_compute_show_cancel_wizard_button',
        store=False,
    )

    def _compute_show_cancel_wizard_button(self):
        has_right = self.env.user.cancel_rent_same_day
        for rec in self:
            rec.show_cancel_wizard_button = has_right
    ...
    def action_open_cancel_wizard(self):
        """เปิด Wizard ยกเลิก - ถ้าติ๊ก cancel_rent_same_day จะยกเลิกได้ทุกวัน"""
        # ถ้าผ่านการตรวจสอบ ให้เปิด Wizard (ไม่จำกัดวัน เพราะปุ่มจะซ่อนอยู่แล้วถ้าไม่มีสิทธิ์)
        return {
        ...
        try:
            ...
            if self.cancel_sale_order:
                ...
                if self.sale_id.state not in ['cancel', 'done']:
                    try:
                        # Bypass custom permission check (user_cancel_control)
                        # ใช้ SQL โดยตรงเพื่อข้าม allow_cancel check ใน action_cancel()
                        self.env.cr.execute("""
                            UPDATE sale_order
                            SET state = 'cancel'
                            WHERE id = %s
                ...
                for invoice in self.sale_id.invoice_ids:
                    if invoice.state != 'cancel':
                        try:
                            ...
                            if inv.state == 'posted':
                                # Bypass custom permission check (user_cancel_control)
                                # ใช้ direct write เหมือน payment เพื่อข้าม allow_cancel check ใน button_draft()
            ...
            if self.cancel_payment:
                ...
                for inv_name in invoice_names:
                    ...
                    _logger.info("🔵 [CANCEL_PAYMENT_V3] search_invoice_name = '%s' => found %d payment(s): %s",
