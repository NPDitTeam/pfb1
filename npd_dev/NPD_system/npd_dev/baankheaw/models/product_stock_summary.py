...
class ProductStockSummary(models.Model):
    _name = 'baankheaw.product_stock_summary'
    _description = 'สรุปสต๊อกต่อสินค้า (รวมทุกสาขา)'
    _order = 'pdn_id'
    ...
    def action_update_product_summary(self):
        """ปุ่มกดใน Odoo เพื่อดึงข้อมูลใหม่"""
        self.sudo().fetch_and_store_product_summary_data()
        return {
    ...
    @api.model
    def fetch_and_store_product_summary_data(self):
        """
        ...
        try:
            ...
            for pdn_id, summary in product_summary.items():
                # แปลง set เป็น string คั่นด้วยจุลภาค
                branch_names = ', '.join(sorted(summary['branches']))

                self.create({
                    'pdn_id': summary['pdn_id'],
                    'product_name': summary['product_name'],
                    'branch_list': branch_names,  # 🆕 บันทึกรายชื่อสาขา
