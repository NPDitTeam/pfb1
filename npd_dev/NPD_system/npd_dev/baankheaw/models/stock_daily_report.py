...
class StockDailyReport(models.Model):
    ...
    pdn_id = fields.Char(string="รหัสสินค้า")
    ...
    damaged_npd_s_group = fields.Integer(string="สินค้าชำรุดNPD_S_Group")
    damaged_npd_intertrading = fields.Integer(string="สินค้าชำรุดNPD_Intertrading")
    damaged_npd_bangkok = fields.Integer(string="สินค้าชำรุดNPD_Bangkok")
    total_damaged = fields.Integer(
        string="รวมสินค้าชำรุด",
        compute='_compute_total_damaged',
        store=True,
    )
    ...
    @api.depends('damaged_npd_s_group', 'damaged_npd_intertrading', 'damaged_npd_bangkok')
    def _compute_total_damaged(self):
        for rec in self:
            rec.total_damaged = (
                (rec.damaged_npd_s_group or 0)
                + (rec.damaged_npd_intertrading or 0)
                + (rec.damaged_npd_bangkok or 0)
            )
    ...
    def fetch_and_store_stock_report_data(self):
        try:
            ...
            # 🔹 ดึงสินค้าชำรุด NPD_S_Group จาก stock.scrap ของ DB ปัจจุบัน
            damaged_dict = {}
            damaged_reason = self.env['scrap.reason.code'].search(
                [('name', '=', 'สินค้าชำรุด')], limit=1
            )
            if damaged_reason:
                scraps = self.env['stock.scrap'].search([
                    ('reason_code_id', '=', damaged_reason.id),
                    ('state', '=', 'done'),
                    ('company_id', '=', self.env.company.id),
                ])
                for scrap in scraps:
                    bk_reference_code = scrap.product_id.bk_reference_code
                    location_name = scrap.location_id.name
                    if bk_reference_code and location_name:
                        scrap_key = (location_name, bk_reference_code)
                        damaged_dict[scrap_key] = damaged_dict.get(scrap_key, 0) + scrap.scrap_qty

            # 🔹 ดึงสินค้าชำรุดจาก DB อื่นผ่าน JSON-RPC API
            RemoteApi = self.env['npd.remote.scrap.api']
            intertrading_dict = RemoteApi.get_damaged_dict(
                db='NPD_Intertrading_New', login='Npd_admin', password='1234',
            )
            bangkok_dict = RemoteApi.get_damaged_dict(
                db='NPD_Bangkok_New', login='Npd_admin', password='1234',
            )

            # ใส่ค่าให้แต่ละ data_dict
            for key, data in data_dict.items():
                scrap_key = (data['branch_name'], data['pdn_id'])
                data['damaged_npd_s_group'] = int(damaged_dict.get(scrap_key, 0))
                data['damaged_npd_intertrading'] = int(intertrading_dict.get(scrap_key, 0))
                data['damaged_npd_bangkok'] = int(bangkok_dict.get(scrap_key, 0))
