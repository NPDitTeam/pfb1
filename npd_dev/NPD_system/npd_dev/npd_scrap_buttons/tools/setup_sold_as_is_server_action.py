...
# ===========================================================================
#  ใช้เมื่อเข้า shell ของเครื่อง production ไม่ได้ / ต้องรันแยกทีละ DB
#
#  ต้องทำก่อน : อัปเกรดโมดูล npd_scrap_buttons เป็น 14.0.1.3.0 + restart เซิร์ฟเวอร์
#
#  วิธีใช้
#    1. เปิด Developer Mode
#    2. การตั้งค่า -> เทคนิค -> การดำเนินการ -> การดำเนินการเซิร์ฟเวอร์ -> สร้าง
#         ชื่อ   : สร้างคลังขายตามสภาพ
#         โมเดล : Server Action  (ir.actions.server)   <-- สำคัญ!
#         ชนิด   : ดำเนินการโค้ด Python  (Execute Python Code)
#    3. วางโค้ดทั้งไฟล์นี้ลงในช่อง Python Code แล้วบันทึก
#    4. กดปุ่ม "เริ่มทำงาน" -> ขึ้นกล่องบอกว่าจะสร้างอะไรบ้าง (ยังไม่บันทึกจริง)
#    5. ถ้าถูกต้อง กดแก้ไข -> เปลี่ยน DRY_RUN = False -> บันทึก -> เริ่มทำงานอีกครั้ง
#       -> คราวนี้สร้างจริง แล้วเด้งไปหน้ารายการคลังที่สร้างให้ตรวจเลย
#    6. เก็บ Server Action ไว้ใช้ตอนเปิดสาขาใหม่ได้ (ตั้ง DRY_RUN = True กลับ)
#
#  ทำไมโมเดลต้องเป็น Server Action:
#    ปุ่ม "เริ่มทำงาน" ในฟอร์มมี attrs ซ่อนไว้ว่าโมเดลต้องเป็น ir.actions.server
#    ถ้าเลือกโมเดลอื่น ปุ่มจะไม่โผล่ กดรันไม่ได้
#
