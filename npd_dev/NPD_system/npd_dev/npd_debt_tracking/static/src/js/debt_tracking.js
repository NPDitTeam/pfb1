odoo.define('npd_debt_tracking.call_action', function (require) {
    "use strict";
    ...
    FormController.include({
        ...
        _onButtonClicked: function (ev) {
            ...
            if (buttonName === 'action_dial_now' && this.modelName === 'npd.debt.tracking.call.log') {
                ev.stopPropagation();
                
                var record = this.model.get(this.handle);
                var recordId = record.data.id;
                
                console.log('NPD: Dial clicked, ID:', recordId);
                
                // เรียก RPC เพื่อบันทึกเวลาและรับ phone number
                rpc.query({
                    model: 'npd.debt.tracking.call.log',
                    method: 'js_start_call',
                    args: [[recordId]],
                }).then(function (result) {
                    console.log('NPD: Result:', result);
    ...
    console.log('NPD: Call Action Loaded Successfully');
});
