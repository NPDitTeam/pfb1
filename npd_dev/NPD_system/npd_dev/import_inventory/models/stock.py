...
class gen_inv(models.TransientModel):
                ...
                if self.location_id_option == True : 
                    if i!= 0:
                        ...
                        if bool(val):
                            ...
                            search_line = self.env['stock.inventory.line'].search([('product_id','=',val['product']),('inventory_id','=',inventory_id.id),('location_id','in',stock_location_id.ids)])
                            if search_line :
    ...
    def download_auto(self):
        return {
             ...
             'url': '/web/binary/download_document?model=gen.inv&id=%s'%(self.id),
             'target': 'new',
             }
