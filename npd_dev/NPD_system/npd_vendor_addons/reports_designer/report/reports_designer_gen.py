...
def dc(obj): 
    if isinstance(obj, dict):
        ...
        d = obj 
    return d
class ReportsDesignerGen(models.TransientModel):
    _name = 'reports_designer_gen'
    _description = " "
    ...
    def _get_template(self, template_id):
        return self.env['ir.attachment'].search([('res_model', '=', 'reports.designer'), ('id', '=', template_id)])
    def _get_conf(self, conf_id):
        return self.env['reports.designer'].browse(conf_id)
    ...
    def create_xls(self, datas=None, cellutil=None):
        ...
        res_conf = {}
        res_conf = self.sudo()._get_conf(datas['form']['report_conf_id']) or {}
        if not res_conf:
        ...
        for i in res_conf.reports_designer_param_ids:
        ...
        for i in res_conf.reports_designer_section_ids:
        ...
        if datas['send_by_email']:
            ...
            if template and template.lang and len(datas['ids']):
                lang = template._render_lang(datas['ids'])[datas['ids'][0]]
            ctx = {
            ...
            if len(datas['ids']):
                ctx['default_res_id'] = datas['ids'][0]
            else:
                ctx['default_res_id'] = datas['form']['id']
                ctx['default_model'] = 'reports_designer_wizard'
            return {
            ...
            file_obj = {
                ...
                'url': '/reports_designer?id=%s' % (new_attach.id,), 
                'target': 'self',
            }
            return file_obj
    def _get_section_conf(self, section_id, params=None, xlsx_template=None, chain_group=None, Drawing_ML=None):
        conf_section = {}
        section = self.env['reports.designer.section'].sudo().search_read([('id','=', section_id)],[])[0]
        chain_group_ext = False
        fields = self.env['reports.designer.fields'].sudo().search_read([('id', 'in', section['reports_designer_fields_ids'])],[])
        for f in fields:
            if f['show']:
        ...
        conf_section  = {
            ...
            'reports_designer_fields_ids': {d[0][0]:d for d in ([self._get_fields_conf(p, params, xlsx_template, Drawing_ML) for p in section['reports_designer_fields_ids']] if len(section['reports_designer_fields_ids']) else section['reports_designer_fields_ids'])},
            'sql_bool': section['sql_bool'],
            'reports_designer_fields_sql_ids': section['reports_designer_fields_sql_ids'],
    ...
    def _get_fields_conf(self, field_id, params=None, xlsx_template=None, Drawing_ML=None):
        conf_field = []
        fields = self.env['reports.designer.fields'].browse(field_id)
        for n in fields:
            having_value = None
            if (n.having_operator != False):
                if (n.having_selection == 'param'):
                    res_param = self.env['reports.designer.param'].browse(n.having_param_id.id)
                    for k,v in list(params.items()):
                        if res_param.id == v[0]: 
                            if v[4]:  
            ...
            conf_field = [self._get_conf_coordinate(xlsx_template, n.cell), 
                          ...
                          n.reports_designer_section_id.id,    
                          {
                          'group_by': n.group_by,
    ...
    def _get_field_data(self, res, conf, xlsx_template, Models_Env, Fields_Env, lang):
        val = {}
        for k, v in list(conf['reports_designer_fields_ids'].items()):
    ...
    def _post_processing_data(self, section_id, conf=None, data=None, parent_chain_ids=None, having_remove_ids=None):
        ...
        for k, v in list(sorted(conf_active['reports_designer_fields_ids'].items(), key = lambda x : x[1][4]['sequence'])):
            if v[4]['formula']:
                for dl in data_active['data']:
                    ...
                    dl[3][k][1] = res_value 
        group_by = []
        for k, v in list(conf_active['reports_designer_fields_ids'].items()):
            if v[4]['group_by']:
        ...
        if len(sort_by_group):
            ...
            for s in sort_by_group_sorted:
                get_val_sort_group_f = get_val_sort_group(s[0], conf_active['reports_designer_fields_ids'][s[0]][4]['field_type'])
        ...
        for g in groups:
            ...
            for i in g:
                if not len(group_by):
                    if  len(conf_active['reports_designer_fields_ids']):
                    ...
                    if len(group_res)==0: 
                        ...
                        for k, f in list(i[3].items()):
                            if conf_active['reports_designer_fields_ids'][k][4]['group_by']:
                                continue
                            group_res[0][3][k][1] += f[1]   
            f_get_item_0 = itemgetter(0)
            f_get_item_1 = itemgetter(1)
            for rec_group_res in group_res:
                for ka, va in list(conf_active['reports_designer_fields_ids'].items()):
                    if va[4]['aggregate']:
                        vals = ()
        ...
        having = ()
        for kf, af in  list(conf_active['reports_designer_fields_ids'].items()):
        ...
        if len(having):
            for rec in data_active_res:
                for h in having:
                    rec_val_h = MAP_FIELD_TYPE_BLANK[conf_active['reports_designer_fields_ids'][h[0]][4]['field_type']] if rec[3][h[0]][1][0][1] == '' and conf_active['reports_designer_fields_ids'][h[0]][4]['field_type'] in ('integer', 'float', 'monetary', 'date', 'datetime',) else rec[3][h[0]][1][0][1] 
                    try:
                        if h[1] == "=":
                            if rec_val_h != h[2]:
                        ...
                        msg = ('''ERROR WHEN CALCULATING HAVING FUNCTION IN A CELL "%s" ! \n
                                ...
                                '''
                               ) % (h[0], conf_active['reports_designer_fields_ids'][h[0]][4]['formulas'], e,)
                        raise UserError(msg)      
        ...
        if len(sort_by):
            ...
            for s in sort_by_sorted:
                get_val_sort_f = get_val_sort(s[0], conf_active['reports_designer_fields_ids'][s[0]][4]['field_type'])
                if s[2] == 'asc':
                    reverse = False 
                else:
                    reverse = True 
                try:
                    ...
                    msg = ('''ERROR DURING SORTING CELL "%s" !\n
                            ...
                            Sorting can not be performed on different data types !
                            '''
                           ) % (s[0], conf_active['reports_designer_fields_ids'][s[0]][4]['formulas'], e)
                    raise UserError(msg)                
            ...
            for col in section_cols_range:
                ...
                while row_exist:
                    try:
                        ...
                        row_exist = False
            conf_active['reports_designer_fields_ids'][k][4]['cell_start'] = dc(cell_start)    
        for k, v in list(conf_active['children_ids'].items()):
                    ...
                    for item in conf_active['cell_section_order']:                        
                        if item[0]:
                            ...
                            if len(data_row[3][item[1]][1]) == 1:
                                ...
                                for c_col, val in list(conf_active['reports_designer_fields_ids'][m_cell[0][0]][4]['cell_start'].items()):
    ...
    def _get_model(self, Models_Env, model_name, lang=None):
        if model_name in Models_Env.models:
            ...
            if model_name in ('ir.model', 'ir.model.fields', 'ir.model.data',):
                model_ids = self.env[model_name].sudo().with_context(active_test=False,lang=lang).search([]) if self.env[model_name].sudo()._fields.get('active') else self.env[model_name].sudo().with_context(lang=lang).search([]) 
            else:
