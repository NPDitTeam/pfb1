...
class XLSXEdit(object):
    ...
    def write_conf(self, conf_data):
        ...
        node_cols = xml.xpath('/ws:worksheet/ws:cols', namespaces=self._namespaces)
        self._conf['data_cols_style'] = {}
        if len(node_cols):
            for node_col in node_cols[0]:
                if col_min_ind <= int(node_col.get('min')) <= col_max_ind:
                    if node_col.get('style'):
                        for col in tuple(range(int(node_col.get('min')), int(node_col.get('max')) + 1)):
                            if col <= col_max_ind:
                                self._conf['data_cols_style'][col] = node_col.get('style') 
                else:
                    break
    ...
    def write(self, sheet, cell, value, level="0", category=False, cell_ext=False, DrawingML=False):
        if value not in (True, False, None) and type(value) not in (int, float, str, date, datetime, bytes, Markup):
            raise TypeError('Only None, int, float, str, unicode, bytes, html')
        if DrawingML['field_type'] == 'html':    
            value = self.html_to_text(html.fromstring(Markup(value).unescape()))
    ...
    def _change_cell(self, xml, cell, values):
        ...
        if value == None:
            ...
            try:
                ...
                value = self._str_xml_re.sub('', value)
                value = str(self._add_shared_string(value))
                node_c.attrib['t'] = 's'
        else:
            if node_c.attrib.get('t') == 's':
            ...
            if x_row in self._conf['data_lines_style']:
                if col in self._conf['data_lines_style'][x_row]:
                    if  self._conf['data_lines_style'][x_row][col][1]:
                        ...
                        style_set = True
            if not style_set:
                if cell_col_ind in self._conf['data_cols_style']:
                    node_c.set('s', self._conf['data_cols_style'][cell_col_ind])
        pattern = '/ws:worksheet/ws:mergeCells'
        node_mergeCells = xml.xpath(pattern, namespaces=self._namespaces)
        if len(node_mergeCells):
    ...
    def _remove_calcChain(self, cell = None):
        if self._calcChain and cell is not None: 
            col,row = self.coordinate_from_string(cell)
    ...
    def _add_calc_from_template(self, xml, cell, cell_t):
        ...
        if len(node_rc_n):
            ...
            if node_f_n is not None:
                ...
                f_text_coords = self.all_coordinates_from_string(f_text)
                f_text_coords = sorted(list(set(f_text_coords)))[::-1]
                if len(f_text_coords):
                    f_text_coords_n = []
                    for coord in f_text_coords:
                        x_coord = self.coordinate_from_string_abs(coord)
                    ...
                    i = 0
                    for n_coord in f_text_coords:
                        f_text  = f_text.replace(n_coord,f_text_coords_n[i])
                        i += 1
                    node_f_n.text = f_text
                    if not self._calcChain: 
                        self._init_calcChain()
                    if self._calcChain: 
                        xml_calcChain = self._get_xml('xl/calcChain.xml')
                        ...
                        with open(os.path.join(self._zip_folder, 'xl/calcChain.xml'), 'wb') as calcChain_file_out:
                            xml_calcChain.write(calcChain_file_out)
    def _init_calcChain(self):
        try:
            self._get_xml('xl/calcChain.xml')
            self._calcChain = True
        except:
            calcChain_xml = etree.ElementTree(etree.fromstring(
                '''<calcChain xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"></calcChain>'''))
            with open(os.path.join(self._zip_folder, 'xl/calcChain.xml'), 'wb') as calcChain_out:
                calcChain_xml.write(calcChain_out,
                                    xml_declaration=True, 
                                    encoding="UTF-8", 
                                    standalone="yes")
            Content_Types_xml = self._get_xml('[Content_Types].xml')
            Content_Types_root = Content_Types_xml.xpath('/ct:Types', namespaces=self._namespaces)[0]
            Content_Types_root.append(etree.Element('Override', PartName="/xl/calcChain.xml", ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.calcChain+xml"))
            with open(os.path.join(self._zip_folder, '[Content_Types].xml'), 'wb') as Content_Types_out:
                Content_Types_xml.write(Content_Types_out,
                                    xml_declaration=True, 
                                    encoding="UTF-8", 
            ...
            pattern_params = {'Target': 'calcChain.xml'}
            pattern_r = '/rel:Relationships/rel:Relationship[@Target="%(Target)s"]' % pattern_params
            node_r_calcChain = workbook_xml_rels_xml.xpath(pattern_r, namespaces=self._namespaces)
            if not len(node_r_calcChain) :   
                workbook_xml_rels_root = workbook_xml_rels_xml.xpath('/rel:Relationships', namespaces=self._namespaces)[0]
                r_id_max = 0
                for node in workbook_xml_rels_xml.xpath('/rel:Relationships/rel:Relationship', namespaces=self._namespaces):
                    r_id = node.attrib['Id']
                    ...
                    r_id_max = n_id if n_id > r_id_max else r_id_max
                r_id_new = "rId"+ str(r_id_max + 1) 
                workbook_xml_rels_root.append(etree.Element('Relationship', Id=r_id_new, Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/calcChain", Target="calcChain.xml"))
                with open(os.path.join(self._zip_folder, 'xl/_rels/workbook.xml.rels'), 'wb') as workbook_xml_rels_out:
                    workbook_xml_rels_xml.write(workbook_xml_rels_out,
                                        xml_declaration=True, 
                                        encoding="UTF-8", 
                        ...
                        if len(xn):
                            if len(xn) == 1:
                                ...
                                if row_1 >= row_start:
                                    ...
                                    attr_sqref_split_n.append(attr_sqref_x)
                            elif len(xn) == 2:
                                col_1i,row_1,col_2i,row_2 = self.range_boundaries(attr_sqref_x)
                                col_1 = self.get_column_letter(col_1i)
                                col_2 = self.get_column_letter(col_2i)
                                if row_1 >= row_start:
                                    ...
                                    attr_sqref_split_n.append(attr_sqref_x)
                    node_cf.set('sqref', ' '.join(attr_sqref_split_n))                                         
                    if self.coordinate_from_string(self.all_coordinates_from_string(attr_sqref)[0])[1] >= row_start:
                        node_cf_childs = node_cf.getchildren()
                        if len(node_cf_childs):
                            for cfRule in node_cf_childs:
                                node_formula = cfRule.find('ws:formula', namespaces=self._namespaces)
                ...
                for definedName in node_definedNames:
                    ...
                    if f_text.find(sheet_name) >= 0 : 
                        ...
                        if len(coordinates):
                            ...
                            with open(os.path.join(self._zip_folder, 'xl/workbook.xml'), 'wb') as workbook_out:
                                workbook_xml.write(workbook_out)
    def html_to_text(self, element):
        output = []
        self._wrap(element, output)
        return re.sub(
            r'[ \t\r\f]*\n[ \t\r\f]*',
            '\n',
            ''.join(self._realize_padding(output)).strip())
    ...
    def _realize_padding(self, it):
        ...
        for item in it:
            ...
            if padding:
                ...
                padding = 0
            yield item
    def _wrap(self, element, output, wrapper=''):
        output.append(wrapper)
        if element.text:
            output.append(self._collapse_whitespace(element.text))
        for child in element:
            self._element_to_text(child, output)
        output.append(wrapper)
    def _element_to_text(self, e, output):
        if e.tag == 'br':
            output.append('\n')
        elif e.tag in self._PADDED_BLOCK:
            self._wrap(e, output, 2)
        elif e.tag in self._MISC_BLOCK:
            self._wrap(e, output, 1)
        else:
            self._wrap(e, output)
        if e.tail:
            output.append(self._collapse_whitespace(e.tail))
