odoo.define('pfb_list_view_sequence.ListNumber', function (require) {
"use strict";
...
ListRenderer.include({
    ...
    // NOTE: _renderGroupRow use for add sequence to main tree view
    //
    ...
    //     return $row;
    // },
    // NOTE: _renderGroups use for add sequence to main tree view
    //
    ...
    //                 		return self._renderRow(record).prepend($("<th class='o_list_number'>").html(index+1)); //.prepend($('<td>'));
    //                 	}
    //                 	else{
    ...
    _renderHeader: function (isGrouped) {
    	...
    	if (this.hasSelectors) {
            // NOTE: $header.find use for add sequence to main tree view
    		// $header.find("th.o_list_record_selector").before($('<th class="o_list_row_number_header o_list_number">').html('#'));
    		...
    		if (this.mode !== 'edit'){
    			$header.find("tr").prepend($("<th class='o_list_row_number_header o_list_number'>").html('#'));
    		}
    	}
    	//$header.find("tr").prepend($('<th>').html('#'));
    	return $header;
    },
    // NOTE: _renderRow use for add line sequence in form view -> tree inside the form (e.g., product_line)
    _renderRow: function (record) {
    	var $row = this._super(record);
        // NOTE: to correction the current view as list and parent view is form
        if (this.viewType === "list" && this.getParent().viewType === 'form'){
    	// if (this.mode !== 'edit' && this.state.groupedBy.length==0){
	    	var index = this.state.data.findIndex(function(e){return record.id===e.id})
	    	if (index!==-1){
	    		$row.prepend($("<th class='o_list_number'>").html(index+1));
	    	}
    	}
    	return $row;
    },
