...
class InsGeneralLedgerXlsx(models.AbstractModel):
    _name = 'report.account_dynamic_reports_custom.ins_general_ledger_xlsx'
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.write_string(self.row_pos, 3, _('Analytic Accounts'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 4, _('Analytic Tag'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Department'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Fund'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 7, _('Job Order'),
                                    self.format_header)

                                    
            # self.sheet.write_string(self.row_pos, 3, _('Ref'),
            #                         self.format_header)
            self.sheet.write_string(self.row_pos, 8, _('Move'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 9, _('Entry Label'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 10, _('Debit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 11, _('Credit'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 12, _('Balance'),
                                    self.format_header)
        else:
        ...
        if acc_lines:
            for line in acc_lines:
                ...
                self.sheet.write_number(self.row_pos, 10, float(acc_lines[line].get('debit')), self.line_header)
                self.sheet.write_number(self.row_pos, 11, float(acc_lines[line].get('credit')), self.line_header)
                self.sheet.write_number(self.row_pos, 12, float(acc_lines[line].get('balance')), self.line_header)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            self.row_pos += 1
                            self.sheet.write_string(self.row_pos, 9, sub_line.get('move_name'),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 10, float(acc_lines[line].get('debit')),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 11, float(acc_lines[line].get('credit')),
                                                    self.line_header_light_initial)
                            self.sheet.write_number(self.row_pos, 12, float(acc_lines[line].get('balance')),
                                                    self.line_header_light_initial)
                            ...
                            self.sheet.write_string(self.row_pos, 3, sub_line.get('analytic_ids') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 4, sub_line.get('analytic_tag_ids') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 5, sub_line.get('department_id') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 6, sub_line.get('fund_id') or '',
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 7, sub_line.get('job_order_id') or '',
                                                    self.line_header_light)
                            # self.sheet.write_string(self.row_pos, 3, sub_line.get('lref') or '',
                            #                         self.line_header_light)
                            self.sheet.write_string(self.row_pos, 8, sub_line.get('move_name'),
                                                    self.line_header_light)
                            self.sheet.write_string(self.row_pos, 9, sub_line.get('lname') or '',
                                                    self.line_header_light)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet.set_column(10, 10, 10)
        self.sheet.set_column(11, 11, 10)
        self.sheet.set_column(12, 12, 10)

        self.sheet_2.set_column(0, 0, 35)
        self.sheet_2.set_column(1, 1, 25)
        self.sheet_2.set_column(2, 2, 25)
        
        self.sheet_2.set_column(8, 8, 25)
        self.sheet_2.set_column(9, 9, 25)
        self.sheet_2.set_column(10, 10, 25)
        self.sheet_2.set_column(11, 11, 25)
