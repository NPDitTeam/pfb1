...
class ins_account_financial_report(models.Model):
    ...
    def _get_children_by_order(self, strict_range):
        ...
        if not strict_range:
            res -= self.env.ref('account_dynamic_reports_custom.ins_account_financial_report_unallocated_earnings0')
            res -= self.env.ref('account_dynamic_reports_custom.ins_account_financial_report_equitysum0')
        return res
...
class AccountAccount(models.Model):
    ...
    def get_cashflow_domain(self):
        cash_flow_id = self.env.ref('account_dynamic_reports_custom.ins_account_financial_report_cash_flow0')
        if cash_flow_id:
    ...
    def onchange_cash_flow_category(self):
        ...
        if self._origin and self._origin.id:
            ...
            self.env.ref(
                'account_dynamic_reports_custom.ins_account_financial_report_cash_flow0').write(
