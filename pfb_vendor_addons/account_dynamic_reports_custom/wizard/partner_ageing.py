...
class InsPartnerAgeing(models.TransientModel):
        ...
        for partner in partner_ids:
            ...
            sql = """
                ...
                LEFT JOIN
                    account_account_type AS ty ON a.user_type_id = ty.id
                LEFT JOIN 
                    account_analytic_account anl ON (l.analytic_account_id=anl.id)    
                WHERE
                    l.balance <> 0
                    AND m.state = 'posted'
                    AND ty.type IN %s
                    ...
                    AND l.company_id = %s
                    %s
            """%(type, partner.id, as_on_date, company_id.id,WHERE)
            self.env.cr.execute(sql)
            ...
            if count:
                for period in period_dict:
                    ...
                    amount = 0.0
                    self.env.cr.execute(sql + where + WHERE)
    ...
    def get_xlsx_report(self, data, response):
        ...
        if record.include_details:
            ...
            sheet.write(row_pos, 4, _('Analytic Accounts'), format_header)
            sheet.write(row_pos, 5, _('Analytic Tag'), format_header)
            sheet.write(row_pos, 6, _('Department'), format_header)
            sheet.write(row_pos, 7, _('Fund'), format_header)
            sheet.write(row_pos, 8, _('Job Order'), format_header)
        ...
        if ageing_lines:
            for line in ageing_lines:
                ...
                row_pos += 1
                if line != 'Total':
                    sheet.merge_range(row_pos, 0, row_pos, 8, ageing_lines[line].get('partner_name'), line_header)
                else:
                    sheet.merge_range(row_pos, 0, row_pos, 8, _('Total'),line_header_total)
                k = 9
                for period in period_list:
                    if line != 'Total':
                ...
                if record.include_details:
                    if line != 'Total':
                        ...
                        for sub_line in sub_lines:
                            ...
                            sheet.write(row_pos, 4, sub_line.get('account_analytic_name') or '', line_header_light)
                            sheet.write(row_pos, 5, sub_line.get('analtag_name') or '', line_header_light)
                            sheet.write(row_pos, 6, sub_line.get('ldep') or '', line_header_light)
                            sheet.write(row_pos, 7, sub_line.get('lfund') or '', line_header_light)
                            sheet.write(row_pos, 8, sub_line.get('ljob_order') or '', line_header_light)

                            sheet.write(row_pos, 9, float(sub_line.get('range_0')), line_header_light_period)
                            sheet.write(row_pos, 10, float(sub_line.get('range_1')), line_header_light_period)
                            sheet.write(row_pos, 11, float(sub_line.get('range_2')), line_header_light_period)
                            sheet.write(row_pos, 12, float(sub_line.get('range_3')), line_header_light_period)
                            sheet.write(row_pos, 13, float(sub_line.get('range_4')), line_header_light_period)
                            sheet.write(row_pos, 14, float(sub_line.get('range_5')), line_header_light_period)
                            sheet.write(row_pos, 15, float(sub_line.get('range_6')), line_header_light_period)
                            sheet.write(row_pos, 16, '', line_header_light_period)
            row_pos += 1
            k = 9
