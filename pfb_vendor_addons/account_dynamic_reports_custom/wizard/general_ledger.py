...
class InsGeneralLedger(models.TransientModel):
    ...
    job_order_id = fields.Many2one('budget.job.order',string="Job Order")
    ...
    def write(self, vals):
        ...
        if vals.get('department_id') == []:
            vals.update({'department_id': [(5,)]})

        if vals.get('fund_id'):
            vals.update({'fund_id': vals.get('fund_id')})
        if vals.get('fund_id') == []:
            vals.update({'fund_id': [(5,)]})

        if vals.get('job_order_id'):
            vals.update({'job_order_id': vals.get('job_order_id')})
        if vals.get('job_order_id') == []:
            vals.update({'job_order_id': [(5,)]})
    ...
    def process_filters(self):
        ...
        if data.get('department_id'):
            filters['department_id'] = data.get('department_id')
        else:
            filters['department_id'] = ''

        if data.get('fund_id'):
            filters['fund_id'] = data.get('fund_id')
        else:
            filters['fund_id'] = ''

        if data.get('job_order_id'):
            filters['job_order_id'] = data.get('job_order_id')
        else:
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            if data.get('department_id'):
                WHERE += ' AND anl.department_id = %s' % data.get('department_id').id

            if data.get('fund_id'):
                WHERE += ' AND l.fund_id = %s' % data.get('fund_id').id                

            if data.get('job_order_id'):
                WHERE += ' AND l.job_order_id = %s' % data.get('job_order_id').id                
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                SELECT
                    ...
                    anl.name as account_analytic_name,
                    tag.name as analtag_name,
                    h.name as ldep,
                    f.name AS lfund,
                    b.name AS ljob_order,
                ...
                LEFT JOIN res_partner p ON (l.partner_id=p.id)
                JOIN account_journal j ON (l.journal_id=j.id)
                LEFT JOIN budget_source_fund f ON (f.id=l.fund_id)
                LEFT JOIN budget_job_order b ON (b.id=l.job_order_id)
                LEFT JOIN account_analytic_tag tag ON tag.id = analtag.account_analytic_tag_id
                LEFT JOIN hr_department h ON h.id = anl.department_id
                
                WHERE %s
                GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name,anl.name,tag.name,h.name,f.name,b.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            'account_dynamic_reports_custom'
    ...
    def get_xlsx_report(self, data, response):
        ...
        if filter.get('include_details', False):
            ...
            sheet.write_string(row_pos, 3, _('Analytic Accounts'), format_header)
            sheet.write_string(row_pos, 4, _('Analytic Tag'), format_header)
            sheet.write_string(row_pos, 5, _('Department'), format_header)
            sheet.write_string(row_pos, 6, _('Fund'), format_header)
            sheet.write_string(row_pos, 7, _('Job Order'), format_header)

            # self.sheet.write_string(row_pos, 3, _('Ref'),self.format_header)
            sheet.write_string(row_pos, 8, _('Move'), format_header)
            sheet.write_string(row_pos, 9, _('Entry Label'), format_header)
            sheet.write_string(row_pos, 10, _('Debit'), format_header)
            sheet.write_string(row_pos, 11, _('Credit'), format_header)
            sheet.write_string(row_pos, 12, _('Balance'), format_header)
        else:
        ...
        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 9, '            ' + account_lines[line].get('code') + ' - ' + account_lines[line].get('name'), line_header_left)
                sheet.write(row_pos, 10, float(account_lines[line].get('debit')), line_header)
                sheet.write(row_pos, 11, float(account_lines[line].get('credit')), line_header)
                sheet.write(row_pos, 12, float(account_lines[line].get('balance')), line_header)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            row_pos += 1
                            sheet.write(row_pos, 9, sub_line.get('move_name'), line_header_light_initial_bold)
                            sheet.write(row_pos, 10, float(sub_line.get('debit')), line_header_light_initial)
                            sheet.write(row_pos, 11, float(sub_line.get('credit')), line_header_light_initial)
                            sheet.write(row_pos, 12, float(sub_line.get('balance')), line_header_light_initial)
                            ...
                            sheet.write(row_pos, 3, sub_line.get('account_analytic_name') or '', line_header_light)
                            sheet.write(row_pos, 4, sub_line.get('analtag_name') or '', line_header_light)
                            sheet.write(row_pos, 5, sub_line.get('ldep') or '', line_header_light)
                            sheet.write(row_pos, 6, sub_line.get('lfund') or '', line_header_light)
                            sheet.write(row_pos, 7, sub_line.get('ljob_order') or '', line_header_light)
                            # sheet.write_string(row_pos, 3, sub_line.get('lref') or '', line_header_light)
                            sheet.write(row_pos, 8, sub_line.get('move_name'), line_header_light)
                            sheet.write(row_pos, 9, sub_line.get('lname') or '', line_header_light)
                            sheet.write(row_pos, 10, float(sub_line.get('debit')),line_header_light)
                            sheet.write(row_pos, 11, float(sub_line.get('credit')),line_header_light)
                            sheet.write(row_pos, 12, float(sub_line.get('balance')),line_header_light)
                        else: # Ending Balance
                            row_pos += 1
                            sheet.write(row_pos, 9, sub_line.get('move_name'), line_header_light_ending_bold)
                            sheet.write(row_pos, 10, float(account_lines[line].get('debit')), line_header_light_ending)
                            sheet.write(row_pos, 11, float(account_lines[line].get('credit')), line_header_light_ending)
                            sheet.write(row_pos, 12, float(account_lines[line].get('balance')), line_header_light_ending)
