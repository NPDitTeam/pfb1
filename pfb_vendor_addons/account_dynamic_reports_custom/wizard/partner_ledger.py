...
class InsPartnerLedger(models.TransientModel):
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                ...
                LEFT JOIN budget_job_order b ON (l.job_order_id = b.id)
                LEFT JOIN hr_department h ON h.id = anl.department_id
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                GROUP BY l.id, l.partner_id, a.name, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name ,anl.name,h.name,f.name,b.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def get_xlsx_report(self, data, response):
        ...
        if filter.get('include_details', False):
            ...
            sheet.write(row_pos, 3, _('Analytic Accounts'),
                                    format_header)
            sheet.write(row_pos, 4, _('Analytic Tag'),
                                    format_header)
            sheet.write(row_pos, 5, _('Department'),
                                    format_header)
            sheet.write(row_pos, 6, _('Fund'),
                                    format_header)
            sheet.write(row_pos, 7, _('Job Order'),
                                    format_header)


            # sheet.write(row_pos, 3, _('Ref'),
            #                         format_header)
            sheet.write(row_pos, 8, _('Move'),
                                    format_header)
            sheet.write(row_pos, 9, _('Entry Label'),
                                    format_header)
            sheet.write(row_pos, 10, _('Debit'),
                                    format_header)
            sheet.write(row_pos, 11, _('Credit'),
                                    format_header)
            sheet.write(row_pos, 12, _('Balance'),
                                    format_header)
        else:
        ...
        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 9, account_lines[line].get('name'), line_header)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            row_pos += 1
                            sheet.write(row_pos, 9, sub_line.get('move_name'),
                                                    line_header_light_initial_bold)
                            sheet.write(row_pos, 10, float(sub_line.get('debit')),
                                                    line_header_light_initial)
                            sheet.write(row_pos, 11, float(sub_line.get('credit')),
                                                    line_header_light_initial)
                            sheet.write(row_pos, 12, float(sub_line.get('balance')),
                                                    line_header_light_initial)
                            ...
                            sheet.write(row_pos, 8, sub_line.get('move_name'),
                                                    line_header_light)
                            sheet.write(row_pos, 9, sub_line.get('lname') or '',
                                                    line_header_light)
                            ...
                            sheet.write(row_pos, 12,
                                                    float(sub_line.get('balance')),line_header_light)
                        else: # Ending Balance
                            row_pos += 1
                            sheet.write(row_pos, 9, sub_line.get('move_name'),
                                                    line_header_light_ending_bold)
                            sheet.write(row_pos, 10, float(account_lines[line].get('debit')),
                                                    line_header_light_ending)
                            sheet.write(row_pos, 11, float(account_lines[line].get('credit')),
                                                    line_header_light_ending)
                            sheet.write(row_pos, 12, float(account_lines[line].get('balance')),
                                                    line_header_light_ending)
