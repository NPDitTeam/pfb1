...
class account_move(models.Model):
    ...
    def _compute_discount_amount_computed(self):
        """
        Calculate total discount from all lines in Invoice
        Sum of discount_amount_computed from each line
        """
        for move in self:
            total_discount = 0.0
            for line in move.invoice_line_ids:
                if line.discount_method == 'fix':
                    # If fix, add amount as entered
                    total_discount += line.discount_amount
                elif line.discount_method == 'per':
                    # If per, calculate as amount from total price
            ...
            for rec in self:
                if res_config.tax_discount_policy == 'tax':
                    ...
                    # rec.amount_total_signed = abs(rec.amount_total) if move.move_type == 'entry' else sign * (-rec.amount_total)
                        ...
                        rec.amount_total = rec.amount_tax + rec.amount_untaxed
                        rec.amount_total_signed = abs(rec.amount_total) if move.move_type == 'entry' else sign * (
                            -rec.amount_total)
                                        ...
                                        for tax_ids in line.tax_ids:
                                            if tax_ids.price_include:
                                                ...
                                                if line.discount_amount > 0:
                                                    line.discount_amt = (line.price_unit * line.quantity) * (100 / (
                                                                100 + taxes_amount_include)) * line.discount_amount / 100
                                            else:
                                                # not calculate WTH -3 % on PO
                                                if tax_ids.amount > 0:
                                                ...
                                                if line.discount_amount > 0:
                                                    line.discount_amt = (
                                                                                    line.quantity * line.price_unit) * line.discount_amount / 100
                                    ...
                                    if line.price_unit and line.quantity:
                                        if line.discount_amount:
                                            ...
                                            if line.tax_ids:
                                                ...
                                                taxes_res = line.tax_ids.compute_all(price_unit_discount, cur_obj,
                                                ...
                                                line.price_subtotal = (
                                                                                  line.quantity * line.price_unit) - line.discount_amount
    ...
    discount_amount_computed = fields.Float(
        string='Computed Discount', 
        compute='_compute_discount_amount_computed', 
        store=True,
        digits='Discount',
        help='Calculate discount amount based on discount_method: fix = amount as entered, per = calculated from total price'
    )
    ...
    amount_total = fields.Monetary(string='Total', digits='Account', store=True, readonly=True,
                                   compute='_compute_amount')
    ...
    amount_price_subtotal_without_discount = fields.Float(compute='_compute_amount', string="Subtotal without Discount",
                                                          readonly=True, store=True)
    amount_price_total_full = fields.Float(
        compute='_compute_amount_price_total_full', 
        string="Total Amount (Full)", 
        store=True,
        digits='Product Price',
        help='Total full amount from all lines without discount and tax: Sum of (Price Unit × Quantity)'
    )

    @api.depends('invoice_line_ids.price_total_full')
    def _compute_amount_price_total_full(self):
        """
        Calculate total full amount by summing price_total_full from all invoice lines
        """
        for move in self:
            total_full = 0.0
            for line in move.invoice_line_ids:
                total_full += line.price_total_full
            move.amount_price_total_full = total_full
        ...
        def _compute_base_line_taxes(base_line):
            ...
            if move.is_invoice(include_receipts=True):
                ...
                price_unit_wo_discount = sign * base_line.price_unit * (
                            1 - (base_line.discount / 100.0) * base_line.quantity)
                            ...
                            if base_line.discount_method == 'fix':
                                ...
                                for tax_ids in base_line.tax_ids:
                                    if tax_ids.price_include:
                                        ...
                                        if tax_ids.amount > 0:
                                            taxes_amount_execlude = tax_ids.amount
                                stwd = (base_line.price_unit * base_line.quantity) * (
                                            100 / (100 + taxes_amount_include))
                                if stwd > 0:
                                ...
                                price_unit_discount = base_line.price_unit * (
                                            1 - (base_line.discount_amount or 0.0) / 100.0)
...
class account_move_line(models.Model):
    ...
    price_total_full = fields.Float(
        string='Total Amount (Full)',
        compute='_compute_price_total_full',
        store=True,
        digits='Product Price',
        help='Full amount without discount and tax: Price Unit × Quantity'
    )
    price_subtotal_without_discount = fields.Float(compute='_compute_amount', string="Subtotal without Discount",
                                                   readonly=True, store=True)
    discount_amount_computed = fields.Float(
        string='Computed Discount Amount', 
        compute='_compute_discount_amount_computed', 
        store=True,
        digits='Discount',
        help='Calculate discount amount based on discount_method: fix = amount as entered, per = calculated from total price'
    ...
    def _compute_price_total_full(self):
        """
        Calculate full amount without discount and tax
        Formula: Price Unit × Quantity
        """
        for line in self:
            line.price_total_full = line.price_unit * line.quantity

    @api.depends('discount_method', 'discount_amount', 'price_unit', 'quantity', 'price_subtotal')
    def _compute_discount_amount_computed(self):
        """
        Calculate discount amount based on discount_method:
        - fix: Use amount as entered in discount_amount
        - per: Calculate as amount from total price (price_subtotal * discount_amount / 100)
        """
        for line in self:
            if line.discount_method == 'fix':
                # If fix, show amount as entered
    ...
    @api.onchange('quantity', 'discount', 'price_unit', 'tax_ids', 'discount_method', 'discount_amount',
                  'discount_type', 'price_subtotal_without_discount')
                ...
                if line.discount_amount:
                    if line.price_unit and line.quantity:
                        if line.tax_ids:
                            ...
                            # discount_percent = (line.discount_amount/((line.quantity * line.price_unit)))*100
                            price_unit_discount = line.price_unit * (1 - (discount_percent or 0.0) / 100.0)
                            taxes_res = line.tax_ids.compute_all(price_unit_discount, cur_obj, line.quantity,
                                                                 line.product_id)
                            # price_unit_wo_discount
                    ...
                    for tax_ids in line.tax_ids:
                        if tax_ids.price_include:
                            ...
                            if tax_ids.amount > 0:
                                taxes_amount_include = tax_ids.amount
                            if line.discount_amount > 0:
                                line.discount_amt = (line.price_unit * line.quantity) * (
    ...
    @api.depends('quantity', 'discount', 'price_unit', 'tax_ids', 'discount_method', 'discount_amount', 'discount_type',
                 'price_subtotal_without_discount')
    def _compute_amount(self):
        """
            ...
            if res_config:
                if res_config.tax_discount_policy == 'untax':
                            ...
                            if line.tax_ids:
                                for tax_ids in line.tax_ids:
                                    if tax_ids.price_include:
                                        ...
                                        if tax_ids.amount > 0:
                                            taxes_amount_execlude = tax_ids.amount
                            taxes_res = line.tax_ids.compute_all(line.price_unit, cur_obj, line.quantity,
                                                                 line.product_id)
                            stwd = 0.0
                            stwd = (line.price_unit * line.quantity) * (100 / (100 + taxes_amount_include))
                            line.price_subtotal_without_discount = stwd
                    ...
                    if line.discount_type == 'line':
                        price_x = 0.0
                        price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
                        taxes = line.tax_ids.compute_all(price, line.move_id.currency_id, line.quantity,
                                                         product=line.product_id,
                                                         partner=line.move_id.partner_shipping_id)
