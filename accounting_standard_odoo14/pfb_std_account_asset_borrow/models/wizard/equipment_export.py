...
class assetEquipmentExport(models.Model):
    _name = "asset.equipment.export"
    ...
    #     [
    #         ('cho_muon', 'Borrow(Require)summary table'),
    #         ('dieu_chuyen', 'Borrow(Transfer) summary table'),
    #         ('provide', 'Provide summary table')
    #     ],
    #     string='Type Export',
    #     default="cho_muon")
    export_type = fields.Selection(
        [
            ('dieu_chuyen', 'Borrow summary table'),
            ('provide', 'Provide summary table')
        ],
        string='Type Export',
        default="dieu_chuyen")
    ...
    def print_excel_report(self):
        ...
        return {
            ...
            'res_model': 'asset.equipment.export',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': this.id,
            'views': [(False, 'form')],
    ...
    def generate_equipment(self , export_date):
        if (export_date.start_date == False and export_date.end_date == False):
            equipments = self.env['asset.equipment'].search([])
        elif (export_date.start_date == False and export_date.end_date != False):
            equipments = self.env['asset.equipment'].search([('create_date', '<=', export_date.end_date)])
        elif (export_date.start_date != False and export_date.end_date == False):
            equipments = self.env['asset.equipment'].search([('create_date', '>=', export_date.start_date)])
        else:
            equipments = self.env['asset.equipment'].search(
    ...
    def generate_cho_muon_dieu_chuyen(self , export_date, borrow_type):
        ...
        if (export_date.start_date == False and export_date.end_date == False):
            borrow_requests = self.env['asset.borrow.request'].search([])
        elif (export_date.start_date == False and export_date.end_date != False):
            borrow_requests = self.env['asset.borrow.request'].search([('create_date', '<=', export_date.end_date)])
        elif (export_date.start_date != False and export_date.end_date == False):
            borrow_requests = self.env['asset.borrow.request'].search([('create_date', '>=', export_date.start_date)])
        else:
            borrow_requests = self.env['asset.borrow.request'].search(
                [('create_date', '>=', export_date.start_date), ('create_date', '<=', export_date.end_date)])
        ...
        # Create header
        worksheet.write(0, 0, 'Seq.', style)
        worksheet.write(0, 1, _('Request Date'), style)
        worksheet.write(0, 2, _('Request by'), style)
        worksheet.write(0, 3, _('Asset Name'), style)
        worksheet.write(0, 4, _('Stage'), style)
    ...
    def generate_provide(self , export_date):
        if (export_date.start_date == False and export_date.end_date == False):
            borrow_requests = self.env['asset.provide.request'].search([])
        elif (export_date.start_date == False and export_date.end_date != False):
            borrow_requests = self.env['asset.provide.request'].search([('create_date', '<=', export_date.end_date)])
        elif (export_date.start_date != False and export_date.end_date == False):
            borrow_requests = self.env['asset.provide.request'].search([('create_date', '>=', export_date.start_date)])
        else:
            borrow_requests = self.env['asset.provide.request'].search([('create_date', '>=', export_date.start_date), ('create_date', '<=', export_date.end_date)])
        ...
        worksheet.write(0, 2, _('Request by'), style)
        worksheet.write(0, 3, _('Asset Name'), style)
        worksheet.write(0, 4, _('Status'), style)
